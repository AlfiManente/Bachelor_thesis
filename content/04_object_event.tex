...
\textsc{Powheg} uses the \textsc{NNPDF3.0nlo}\_nf4 PDF \cite{pdf_30} set for ME calculations, while \textsc{Pythia} employs the \textsc{NNPDF2.3lo} PDF set for PS modelling. The normalization  is based on the cross-section prediction of $\sigma^{\rm{SM}}_{\rm{t-chan.}}=\asym{217}{9}{8}{\pico\barn}$ at NLO in QCD \,\cite{t_t_xsec}. \\
%
...
Like the signal process, the \textsc{NNPDF3.0nlo} PDF set is used for ME calculations and the \textsc{NNPDF2.3lo} PDF set for PS modelling. The associated $tW$ produciton is normalized at NLO in QCD to the SM prediction $\sigma^{\rm{SM}}_{tW}=\SI{71.7 \pm 3.8}{\pico\barn}$\,\cite{tw_xsec}.\\
%
