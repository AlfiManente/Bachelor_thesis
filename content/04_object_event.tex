...
In order to measure production cross-sections in collision experiments, it is necessary to distinguish the signal process from the various background contributions.  
...
The formular takes in, the centre-of-mass energy $\sqrt{s}$ of the process, the top quark mass $m_t$, the coupling strength of the strong interaction $\alpha_s$, as well as the factorization scale $\mu_{\rm{F}}$ and the renormalization scale $\mu_{\rm{R}}$, which are set to the energy scale of the process. 
...
Parton showers (PS) simulate the emission QCD radiation, while hadronization models describe the transition from colored partons to colorless hadrons, incorporating the effects of confinement and asymptotic freedom.
...
\textsc{Powheg Box v2}\,\cite{powheg1, powheg2, powheg3} is employed to generate hard scattering processes at NLO accuracy.
Parton showering and hadronization are modelled using either \textsc{Pythia 8}\,\cite{pythia1}, which implements the \textit{Lund string model}\,\cite{lundstring} for hadronization and typically uses the A14 tune\,\cite{A14tune}, or \textsc{Herwig 7} \cite{herwig1,herwig2}, which simulates hadronization based on the \textit{cluster hadronization model}\,\cite{clustermodel}.
...
The decays of heavy-flavor hadrons are simulated using \textsc{EvtGen}\,\cite{evtgen}, except for samples generated with \textsc{Sherpa}.
...
For PS modelling the \textsc{NNPDF2.3lo}\,\cite{pdf_23} PDF set is used. The process is normalized at NLO in QCD to the theoretical cross-section prediction $\sigma^{\rm{SM}}_{\rm{s-chan.}}=\asym{11.073}{0.193}{0.176}{\pico\barn}$\,\cite{t_xsec}. \\
%
...
\textsc{Powheg} uses the \textsc{NNPDF3.0nlo}\_nf4 PDF \cite{pdf_30} set for ME calculations, while \textsc{Pythia} employs the \textsc{NNPDF2.3lo} PDF set for PS modelling. The normalization  is based on the cross-section prediction of $\sigma^{\rm{SM}}_{\rm{t-chan.}}=\asym{214.2}{4.1}{2.6}{\pico\barn}$ at NLO in QCD \,\cite{t_xsec}. \\
%
Associated $tW$ production samples are generated with \textsc{Powheg} interfaced with \textsc{Pythia}, while further making use of the diagram removal scheme\,\cite{diagram_removal} to handle interference with $\ttbar$ production at NLO.
Like the signal process, the \textsc{NNPDF3.0nlo} PDF set is used for ME calculations and the \textsc{NNPDF2.3lo} PDF set for PS modelling. The associated $tW$ produciton is normalized at NLO in QCD to the SM prediction $\sigma^{\rm{SM}}_{tW}=\asym{79.3}{2.9}{2.8}{\pico\barn}$\,\cite{t_xsec}.\\
%
...
The normalization at next-to-next-to-leading order (NNLO) in QCD is calculated to be $\sigma^{\rm{SM}}_{\ttbar}=\asym{833}{37.4}{43}{\pico\barn}$\,\cite{tt_xsec}. \\
%
...
The PS modelling used the \textsc{NNPDF3.0nnlo} PDF set.\\
%
...
Some alternative samples use a faster simulation procedure, where the \textsc{Geant4} simulation of the calorimeter response is replaced by a detailed parameterization of the PS shapes\,\cite{fullsim}, in the following refferet to as \textit{fastsim}.
...
Due to color confinement, quarks hadronize and are reconstructed as jets.
...
Electrons must satisfy the \texttt{TightLH} identification and the tight isolation working point\,\cite{electron_reco}.
Muons are reconstructed by combining tracks from the ID and the MS.
They are required to have $\pT > \SI{10}{\GeV}$ and $|\eta| < 2.5$, and must pass the medium quality selection and the tight isolation working point\,\cite{muon_reco}.
...
Jets are reconstructed using the anti-$k_t$ algorithm\,\cite{antikt1,antikt2} with a radius parameter of $R = 0.4$.
Jets originating from the hard-scatter interaction are identified using the Jet Vertex Tagger (JVT)\,\cite{jvt}, while forward jets ($|\eta| > 2.5$) are validated using the forward JVT (fJVT) algorithm\,\cite{fjvt}.
%which is later used for the forward jet veto.
Jets containing $b$ hadrons are identified using the \textsc{GN2v01} $b$-tagging algorithm\,\cite{btag_algo, btag1, btag2, btag3}.
...
\end{equation*}
is required to exceed $\SI{30}{\GeV}$, suppressing multijet background contributions.
Additional vetos are applied to further reduce background contamination. 
Events containing extra jets with
$\pT > \SI{30}{\GeV}$ or forward jets ($|\eta|>2.5$) are rejected, significantly reducing the single top quark t-channel background.
Events with additional low transverse momentum leptons are discarded to suppress multilepton backgrounds.
All selection criteria defining the signal region are summarized in \autoref{tab:eventsel}.
The number of events passing the event selection is displayed in \autoref{tab:event_yields}
...
\begin{table}
  \centering
  \caption{Number of events passing the event selection for each process inspected in this analysis. \textit{PDG Rounding} is applied to the values\,\cite[18]{pdg}.}
  \label{tab:event_yields}
  \begin{tabular}{l c}
    \toprule
    Process   & Number of events \\
    \midrule
    s-channel                 & 4100 \pm 900 \\
    t-channel                 & 9300 \pm 1800 \\
    $tW$ production           & 2600 \pm 600 \\
    $Z$+jets                  & 1700 \pm 900 \\
    $W$+jets                  & 15000 \pm 8000 \\
    $\ttbar$                  & 75000 \pm 19000 \\
    \midrule
    Total                     &  108000 \pm 25000 \\
    \bottomrule
  \end{tabular}
\end{table}
