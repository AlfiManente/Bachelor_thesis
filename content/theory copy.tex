...
The first category is fermions, which constitute all known matter and carry half-integer spin.  
The second is bosons, acting out out the interactions between fermions and carry a whole integer spin. 
...
\begin{figure}
    ...
    \caption{The fundamental particles of the Standard Model. The boxes illustrate, in which interaction the fermions take part\,\cite{UZH_SM}.}
    \label{sm_table}
\end{figure}

The fermions are comprised of quarks and leptons.
Both interact with the electromagnetic (EM) force but only quarks experience the strong force, while neutrino only interact 
...
The force carrying bosons include the photon ($\gamma$) for the EM interaction, the $\rm{W}^{\pm}$ and $\rm{Z}$ bosons for the weak interaction and eight gluons with different color for the strong interaction. 
...
Gluons, mediating the strong interaction, are massless as well and couple to any particle carrying a color charge, either quarks or glouns themselves. 
There are the three base colors red, green and blue with a respective anticolor counterpart. Gluons always carry two color charges, one base color and one anticolor, resulting in eight different possible permutations. 
Quarks carry only one color charge. 
...
Mesons are color neutral quark-anitquark pairs. Baryons are color neutral composites of three quarks. 
...
The Standard Model (SM) classifies all known fundamental particles into two categories: fermions and bosons.
Fermions carry half-integer spin and constitute the building blocks of matter, while bosons have integer spin and mediate the fundamental interactions.

The fermion sector consists of quarks and leptons.
Leptons participate in the electromagnetic and weak interactions, while quarks additionally experience the strong interaction.
Neutrinos, which belong to the lepton family, are electrically neutral and interact only via the weak force.
Fermions are arranged into three generations, ordered by increasing mass.
The first generation comprises the electron ($e$), the electron neutrino ($\nu_e$), and the up ($u$) and down ($d$) quarks.
The second generation consists of the muon ($\mu$), the muon neutrino ($\nu_\mu$), and the charm ($c$) and strange ($s$) quarks.
The third generation contains the heaviest fermions: the tau lepton ($\tau$) and its neutrino ($\nu_\tau$), as well as the top ($t$) and bottom ($b$) quarks.

Electric charge determines the coupling strength to the electromagnetic interaction.
Charged leptons carry a charge of $-1\,\mathrm{e}$, while up-type quarks ($u, c, t$) have charge $+\sfrac{2}{3}\,\mathrm{e}$ and down-type quarks ($d, s, b$) carry $-\sfrac{1}{3}\,\mathrm{e}$.
Each fermion has a corresponding antiparticle with opposite quantum numbers.
Ordinary matter is predominantly composed of first-generation fermions, with protons and neutrons built from $u$ and $d$ quarks and electrons forming atomic shells.

The force-mediating bosons include the photon ($\gamma$) for the electromagnetic interaction, the $\mathrm{W}^{\pm}$ and $\mathrm{Z}$ bosons for the weak interaction, and eight gluons for the strong interaction.
The photon is massless and electrically neutral and couples to all electrically charged particles.
The weak interaction is mediated by the massive $\mathrm{W}^{\pm}$ and $\mathrm{Z}$ bosons, which leads to its short interaction range.
The $\mathrm{W}^{\pm}$ bosons couple to weak isospin, while the $\mathrm{Z}$ boson couples to a mixture of weak isospin and weak hypercharge.
Gluons are massless and carry color charge, allowing them to interact with quarks and with each other.

The strong interaction is described by Quantum Chromodynamics (QCD), in which quarks carry one of three color charges: red, green, or blue.
Gluons carry a combination of a color and an anticolor, resulting in eight possible states.
Due to color confinement, only color-neutral bound states can exist as free particles.
These bound states, known as hadrons, are classified into mesons, consisting of quark--antiquark pairs, and baryons, composed of three quarks.

The scalar Higgs boson ($H$), proposed in 1964 \cite{higgs_theory} and discovered in 2012 by the ATLAS \cite{higgs_disc1} and CMS \cite{higgs_disc2} collaborations, completes the particle content of the SM.
With spin 0 and no electric charge, it does not mediate a fundamental interaction.
Instead, its coupling to fermions and the $\mathrm{W}^{\pm}$ and $\mathrm{Z}$ bosons generates their masses through the Higgs mechanism.
The measured Higgs boson mass is $m_H = \SI{125.20 \pm 0.11}{\GeV}$ \cite{particle_rev}.
...
Unlike other quarks, which hadronize due to color confinement, the top quark decays before hadronization is possible, due to this short lifetime. 
...
\autoref{tab:prev_measure} summarizes the previous measurements of the single top quark s-channel processes taken at the ATLAS detector. 

\begin{table}
    \centering
    \caption{The three previous measurments of the single top quark s-channel processes taken at the ATLAS detector. Displayed are the cross-sections $\sigma_{s-\rm{chan.}}$, the observed (expected) significance and the integrated luminosity for each respective measurement\,\cite{schan1, schan2, lastmeasure_atlas}.}
    \label{tab:prev_measure}
    \begin{tabular}{l c c c}
        \toprule
        $\sqrt{s}$ & $\sigma_{s-\rm{chan.}}$ & $\sigma_\text{observed} (\sigma_\text{expected})$ & $L_\text{int}$ \\
        \midrule
        \SI{7}{\TeV}    & $ \SI{< \, 26.5}{\pico\barn}$ & --  & $\SI{0.70}{\femto\barn^{-1}} $\\
        \SI{8}{\TeV}    & $4.8 \pm 0.8 (\text{stat.})^{+1.6}_{-1.3}(\text{syst.})\unit{\pico\barn}$ & $ 3.2(3.9)$  & $\SI{20.3}{\femto\barn^{-1}} $\\
        \SI{13}{\TeV}   & $8.2 \pm 0.6 (\text{stat.})^{+3.4}_{-2.8}(\text{syst.})\unit{\pico\barn}$ & $ 3.3(3.9)$  & $\SI{140}{\femto\barn^{-1}} $\\
        \bottomrule
    \end{tabular}
\end{table}

The most recent measurement of the s-channel process was done by ATLAS with a center of mass energy of
...
The analysis strategy consisted of a preselection and then a split of the events into four categories. 
First, the so-called signal region (SR), which is enriched with signal events and used to determine the signal strength.
Three more control regions are inlcuded, depleted in signal events and enriched in background events, to enhance the modelling of the major backgrounds.
These are orthogonal, meaning any event gets put only in one of the regions.
...
The limiting factors of the previous measurement where noticeably the systematic uncertainties, where $\ttbar$ modelling and signal modelling uncertainties, as well as detector modelling uncertainties, had the biggest impact.
\autoref{tab:prev_sys} displays the systematic uncertainties with the largest percentual impact on the signal strength.
The following analysis uses a \textit{Deep Neural Network} discriminant for the profile likelihood fit. 
As it uses a high-dimensional feature set based on kinematic variables to extraxt signal from background events, an improvement on the impact of the systematic uncertaintes and more precise signal extraxtion is expected.


\begin{table}
    \centering
    \caption{The systematic uncertainties whith largest impact on the cross-section measurement of the previous ATLAS measurement of the single top quark s-channel process. The percentual impact of the respective uncertainty on the cross section is shown\,\cite{lastmeasure_atlas}.}
    \label{tab:prev_sys}
    \begin{tabular}{l|c}
        \toprule
        Systematic uncertainty & $\frac{\Delta \sigma}{\sigma} [\%]$ \\
        \midrule
        $\ttbar$ normalisation      & $+24/-17$ \\
        $\ttbar$ shape modelling    & $+18/-15$ \\
        s-channel modelling         & $+18/-8$ \\
        Jet energy resolution       & $+18/-12$ \\
        Jet energy scale            & $+18/-13$ \\
        MC statistics               & $+13/-11$ \\
        Flavour Tagging             & $+12/-10$ \\
        \bottomrule
    \end{tabular}
\end{table}