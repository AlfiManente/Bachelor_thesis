...
The SM is a gauge invariant theory, which describes the fundamental particles and the interactions between them. 
...
The SM classifies all known fundamental particle into two categories:
fermions and bosons. 
...
For every fermion there exists an antiparticle counterpart with quantum numbers. 
...
Gluons, mediating the strong interaction, are massless as well and couple to any particle carrying a color charge, either quarks or gluons themselves. 
...
Color confinement states that every stable composite particle has to be colorless, which is the reason for the finite range of the strong interaction.
...
It mediates none of the fundamental forces, but by coupling to the Higgs field, fermions and the $\rm{W}^{\pm}$ and $\rm{Z}$ boson gain their mass. 
...
In it, a top quark and a bottom bottom quarks are created from a virtual $W$-boson, after the annihilation of a quark and antiquark. 
...
With a theoretical cross-section of  $\sigma_{\ttbar}^{\rm{Theo.}}(\SI{13}{\TeV})=\asym{833.9}{37.4}{43}{\pico\barn}$\,\cite{ttbar_theory_xsec, tt_xsec}, it exceeds every other top quark production mode. 
The other single top quark production modes can be mistaken for signal as well, but to a lesser degree, since their cross sections are noticeably smaller than $\ttbar$ with $\sigma_{t-\rm{chan.}}^{\rm{Theo.}}(\SI{13}{\TeV})=\asym{214.2}{4.1}{2.6}{\pico\barn} $ \cite{t_xsec} and $\sigma_{tW-\rm{Prod.}}^{\rm{Theo.}}(\SI{13}{\TeV})=\asym{79.3}{2.9}{2.8}{\pico\barn}$ \cite{t_xsec}.
Other minor backgrounds stem from multijet background processes, $Z$+jets or diboson ($WW$,$WZ$,$ZZ$) production.
\autoref{fig:background_feyn} shows the leading order (LO) Feynman diagrams for $\ttbar$ production via gluon-gluon fusion, and the other two single top-quark production modes.
...
\autoref{tab:prev_measure} summarizes previous measurements of the single top quark s-channel processes taken at the ATLAS detector. 

\begin{table}
    \centering
    \caption{The two previous measurments of the single top quark s-channel processes taken at the ATLAS detector. Displayed are the cross-sections $\sigma_{s-\rm{chan.}}$, the observed (expected) significance and the integrated luminosity for each measurement\,\cite{schan2, lastmeasure_atlas}.}
...
The cross section of the measured s-channel resulted in $\sigma_\rm{s-chan} = 8.2 \pm 0.6 (\text{stat.})^{+3.4}_{-2.8}(\text{syst.})\unit{\pico\barn}$, in comparison to the theoretical estimate of $\sigma_\rm{s-chan}^{\rm{theo.}}=\asym{10.32}{0.40}{0.36}{\pico\barn}$\,\cite{hathor}. 
...
Three more control regions (CR) are inlcuded, depleted in signal events and enriched in background events, to enhance the modelling of the major backgrounds.
